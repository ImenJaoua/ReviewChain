...
#                   LOAD & FORMAT DATA
# ============================================================
data= load_dataset("comment-train-60k")
valid_data = load_dataset("comment-valid-6k")   # your separate valid set
...
def format_prompt(example):
    ...
    user_input = prompt.format(
        code_diff=example["old_code_diff"].strip(),
                )

    return {
        "prompt": user_input,
        "completion": example["reformulated_comment"].strip()  # ✅ fixed (no loop variable)
    }
