...
from course_project.refinement.prompts.prompt import prompt
...
#                   LOAD & FORMAT DATA
# ============================================================
data= load_dataset("ref-train-100k")
valid_data = load_dataset("ref-valid-10k")   # your separate valid set
...
def format_prompt(example):
    ...
    user_input = prompt.format(
        code_diff=example["old_hunk"].strip(),
        review_comment=example["comment"].strip()  # or example["review_comment"]
    )
    ...
    return {
        "prompt": user_input,
        "completion": example["hunk"].strip()  # ✅ fixed (no loop variable)
    }
...
if continue_from_checkpoint:
    checkpoint_path = os.path.join('./output', "checkpoint-*")
